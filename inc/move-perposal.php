...
if (isset($_POST['proposal_id'])) {
    ...
    if (mysqli_num_rows($po) >= 1) {
        ...
        if ($que = mysqli_query($conn, "INSERT INTO potential (proposal_id, name, email, mo_number, wtp_no, destination, no_person, description, status, amount, sales_person, last_response, travel_date, date, last_activity ,LeadSource) VALUES('$proposal_id', '$name','$email','$mo_number','$wtp_no','$destination','$no_person', '$description', '$status', '$amount', '$sales_person', '$last_response', '$travel_date', '$timestamp', '$timestamp','$LeadSource')")) {
