...
if (isset($_POST["email"]) && strlen($_POST["email"]) > 1) {
  ...
  $res = mysqli_query($db, "SELECT * FROM users WHERE email='" . $_POST["email"] . "'");
  $count  = mysqli_num_rows($res);
  $result = mysqli_fetch_array($res);
  //check email
  if ($result['email'] != $email) {
    echo "<span class='text-danger'>You are not user</span>";
  }
  //check status and email
  elseif ($result['status'] != "0" && $result['email'] == $email) {
    echo "<span class='text-danger'>Hey..! You are blocked by the admin. Please Contact with Admin</span>";
  } else {
