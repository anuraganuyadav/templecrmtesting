...
if(isset($_GET['chat']) && !empty($_GET['chat']))
	{
		$getID = $_GET['chat'];
		$stmt_edit = $DB_con->prepare('SELECT * FROM users WHERE userID=:ugetID');
		$stmt_edit->execute(array(':ugetID'=>$getID));
...
//passing value of chat user
$_SESSION['receivedSMS'] = $user_name;
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
$(document).ready(function() {
	...
	$("#FormSubmit").click(function (e) {
			...
			if($("#sms").val()==='')
			{
		 	...
		 	var ReceivedID = $("#ReceivedID").val(); //build a post data structure
        
			jQuery.ajax({
			type: "POST", // Post / Get method
			url: "inc/caht-response.php", //Where form data is sent on submission
			dataType:"text", // Data type, HTML, json etc.   
            data: {message: message, SMSuserID: SMSuserID, SendPerson: SendPerson, ReceivedPerson: ReceivedPerson, ReceivedID: ReceivedID},      
...
//fetch sms from data base   
 
// $(document).ready(function() {
//			setInterval(function () {
//         //   notification
//				$('#smsFetch').load('Message-fetch.php')
//			}, 1000);
//		});  
...
{
?>           
                <a class="dropdown-item d-flex align-items-center chat-person 
                <?php 
                if($_GET['chat'] == $row['userID']){
                    echo "active";
                }
             else{
                  
                  } 
                 ?>" href="Message-center.php?chat=<?php echo $row['userID'];?>">
                  <div class="dropdown-list-image mr-3">
                   
                 <?php
                  if(empty($row['userPic'])){
                      echo"<img class='rounded-circle img-50' src='images/img/avtar.png'>";
                  }
                  else{   
                    echo "<img class='rounded-circle img-50' src='images/user_images/" . $row['userPic']."' alt='Usr profile'>";
                  }   
                   ?>
...
<!-- show  chat message -->
  ...
  <form id="cmdline">
    ...
    <input type="hidden" value="<?php echo $getID; ?>" id="ReceivedID" name="ReceivedID">
